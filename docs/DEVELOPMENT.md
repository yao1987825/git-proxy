...
## v1.0 — Initial Windows PowerShell version
...
**Features**:
- `-h <IP> -p <PORT> -s` → set SOCKS5 proxy
- `-u` → unset
- `-c` → check status
- `-Help` → help
...
**Key decisions**:
1. **SOCKS5 only** — most flexible; covers HTTP/HTTPS git repos
2. **`--global` flag** — applies to all repos, matches user expectation
3. **Validate inputs** (IP regex, port range) — fail fast on typos
4. **Connectivity test** — TCP connect with 2-second timeout, runs in `-c`

---

## v1.1 — Add HTTP/HTTPS proxy support
...
**Approach 2 — `.cmd` wrapper + PATH** (recommended, works everywhere):
```cmd
@echo off
...
## v1.4 — Debian/Linux bash port

**Driver**: User has a remote Debian 13 server (10.10.10.186), wants the same convenience there.

**Port from PowerShell → bash**:
...
## v1.5 — `.bashrc` corruption disaster (and recovery)
...
In .bashrc:            export PATH=\C:\Users\Administrator/bin:\   ← BUG
```
...
## v2.0 — Project extraction (this repo)
...
├── README.md
├── LICENSE
├── windows/    # PowerShell + CMD wrapper
├── debian/     # bash + installer
├── docs/       # this file + PITFALLS
...
|---------|---------|--------|
| v2.0 | Initial public release | ✅ Done |
...
- Less Git-specific — git has built-in proxy support that integrates with credential helpers, SSL verification, etc.
...
Git supports many authentication methods for proxies (basic, digest, negotiate). We deliberately don't expose these — adding them would make the CLI surface much larger for the 99% case. Advanced users can `git config --global http.proxyAuthMethod 'basic'` manually.
...
|----------|-------|-----------|--------|
| Windows 10 | PowerShell 5.1 | SOCKS5 | ✅ |
| Windows 10 | PowerShell 5.1 | HTTP | ✅ |
| Windows 10 | CMD | SOCKS5 | ✅ |
| Debian 13 | bash 5.2 | SOCKS5 | ✅ |
| Debian 13 | bash 5.2 | HTTP | ✅ |
| Debian 13 | SSH non-interactive | SOCKS5 | ✅ |
| Debian 13 | Login shell | SOCKS5 | ✅ |

---

## Lessons learned

1. **Validate, validate, validate** — input regex catches typos before they corrupt `.gitconfig`
2. **Two protocol tests** (TCP + HTTP) catch more bugs than one
3. **PATH is a hidden dependency** — always make scripts self-sufficient
4. **String escaping across shells** — use file upload, not echo+append
5. **Document every pitfall** — next person (or future-you) will thank you